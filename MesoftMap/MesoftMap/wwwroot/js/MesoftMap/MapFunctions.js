var vm = new Vue({
    ...
    watch: {
        isEditingRegionColor(val) {
            if (!val) {

                return;
            }
            this.$nextTick(function () {   //重新渲染颜色选择器
    ...
    methods: {
        ...
        onSelectRegion(regionID) {
            $("#justFocus")[0].focus();
            if (this.selectedRegion.id) {//原先的选择
        ...
        fillColorPickerInit(e) {
            alert('here, i am here!');
            console.log('fillColorPickerInit:', e);
        },
        onEditRegionColor() {
        ...
        onClickMap(e) {
            ...
            if (!(this.isAddingRegion || this.isEditingRegion)) {
                if (!this.isEditingRegionColor && this.selectedRegion.id && !mapUtility.InOrOutPolygon(this.selectedRegion, latlng)) {
                    this.onSelectRegion(-1);
                }

                if (this.mapDisplayOption.displayLatLng) {  //点击显示坐标
        ...
        onKeyPress(e) {
            ...
            if (value >= 0) {
                ...
                this.setMapTitle(value, mapTitle);
            }
            console.log(e)
        },
        setMapTitle(classID, mapTitle) {
            if (classID < 0) {
                this.map.controls[qq.maps.ControlPosition.BOTTOM_RIGHT].clear();
                return;
            }
            var customZoomDiv = document.createElement("div");
            var innerHtml = OverlayObjectFactory.getImgHTML("/images/maptitle/" + classID + ".jpg", 386, 44, 'NULL');

            customZoomDiv.style.cssText = "padding:5px;border:2px solid #86acf2;background:#ffffff";
            customZoomDiv.index = 1; //设置在当前布局中的位置
            customZoomDiv.innerHTML = classID > -1 ? innerHtml : mapTitle;
            this.map.controls[qq.maps.ControlPosition.BOTTOM_RIGHT].clear();
            this.map.controls[qq.maps.ControlPosition.BOTTOM_RIGHT].push(customZoomDiv);
